...
- [ ] **Unsplash attribution is actually visible on the page, not just in `ATTRIBUTION.md`.** Small and unobtrusive is fine (see `references/illustration-sources.md`); omitted entirely is not — it's a condition of API access, not a style choice, and there is no compliant way to satisfy it with a code comment alone.
- [ ] **Not asset-empty.** Every section that calls for real photography (per `references/component-patterns.md`'s illustration-vs-photo guidance) actually has an Unsplash photo, and every concept-driven section (mission, values, feature explainers) actually has an illustration — not a flat color block or a placeholder standing in for either.
